...
BEGIN_MESSAGE_MAP(CPaintorApp, CWinApp)
	ON_COMMAND(ID_APP_ABOUT, &CPaintorApp::OnAppAbout)
	// 依據文件命令的標準檔案
...
CPaintorApp::CPaintorApp()
{
	...
	SetAppID(_T("Paintor.AppID.NoVersion"));

	// TODO:  在此加入建構程式碼，
	// 將所有重要的初始設定加入 InitInstance 中
}

// 僅有的一個 CPaintorApp 物件
...
BOOL CPaintorApp::InitInstance()
{
	...
	pDocTemplate = new CMultiDocTemplate(IDR_PaintorTYPE,
		RUNTIME_CLASS(CPaintorDoc),
		RUNTIME_CLASS(CChildFrame), // 自訂 MDI 子框架
		RUNTIME_CLASS(CPaintorView));
	if (!pDocTemplate)
		return FALSE;
